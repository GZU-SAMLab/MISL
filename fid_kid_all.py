...
parser.add_argument('--mmd-var', action='store_true', default=True)
...
def calculate_given_paths(real, fake, flag, batch_size, filename, cuda, dims):
    """Calculates the FID of two paths"""
    if not os.path.exists(real):
        raise RuntimeError('Invalid path: %s' % real)
    if not os.path.exists(fake):
        raise RuntimeError('Invalid path: %s' % fake)
    ...
    act1, m1, s1 = _compute_statistics_of_path(real, model, batch_size, dims, cuda)
    print('m1:', m1)
    print('s1:', s1)
    act2, m2, s2 = _compute_statistics_of_path(fake, model, batch_size, dims, cuda)
    print('m2:', m2)
    print('s2:', s2)
    ...
    txt_file.write('\n')
    txt_file.write('FID: %f , KID: %f' % (fid_value, mmd2s.mean() * 100))
    txt_file.write('\n' * 2)


if __name__ == '__main__':  # birds flowers celeba15000 coco
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
        
    for i in range(25, 30):
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('birds_mask-0-15000_Rec-1.0_TGA-2.0_M-0.01_TIM-0.002_G-0.004_LD-2-' + str(i * 10000))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        calculate_given_paths('./images/birds_mask-0-15000_Rec-1.0_TGA-2.0_M-0.01_TIM-0.002_G-0.004_LD-2/real', './images/birds_mask-0-15000_Rec-1.0_TGA-2.0_M-0.01_TIM-0.002_G-0.004_LD-2/only/' + str(i * 10000), str(i * 10000), args.batch_size, 'result/birds_mask-0-15000_Rec-1.0_TGA-2.0_M-0.01_TIM-0.002_G-0.004_LD-2-fid.txt', args.gpu != '', args.dims)
